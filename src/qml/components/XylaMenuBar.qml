...
Item {
    ...
    // Writable on purpose. A readonly binding against MenuBar.implicitWidth
    // fights visible: !isCompactMode and freezes after the first menu rebuild.
    ...
    // --------------------------------------------------------------------------
    // Menu engine — Repeater/Loader cannot populate Menu or MenuBar.
    // Menu is a Popup: Repeater rejects it ("Delegate must be of Item type")
    // and even if it didn't, Repeater children are NOT inserted into
    // Menu.contentModel. Instantiator+createObject(null) is the other trap.
    // We create objects parented to the live Menu and addItem/addMenu ourselves.
    // Top-level File/Edit/View menus are reused; enabled flips only patch items.
    // --------------------------------------------------------------------------
    ...
    function applyItemData(visual, data) {
        if (!visual || !data)
            return
        // XylaMenuItem binds text/enabled/shortcut to itemData — replace the
        // object only. Assigning visual.text would smash those bindings.
    ...
    function addMenuEntry(menu, data) {
        ...
        // CRITICAL FIX: Parent visual items to the Menu's contentItem, not the Menu (Popup) itself.
        // This prevents the "Created graphical object was not placed in the graphics scene" warning.
    ...
    function syncAllMenus() {
        ...
        // Always keep the MenuBar populated — compact detection needs its width.
    ...
    // In-scene parent so createObject() never passes null (that warning) and
    // never parents a Menu into the RowLayout (that blows compact mode).
    Item {
        id: menuIncubator
        width: 0
        height: 0
    ...
    RowLayout {
        ...
        Item {
            ...
            clip: true
            // Stay in the scene when compact so implicitWidth stays valid.
    ...
    // No Instantiator. Nested items are filled by syncMenuItems().
    Component {
        id: submenuComp
        XylaMenu {
