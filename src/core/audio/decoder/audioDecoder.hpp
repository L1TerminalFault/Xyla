...
/**
 * @brief Thread-safe, RAII-governed audio demuxer and decoder based on FFmpeg/libswresample.
 * Converts any audio container/codec (AAC, MP3, FLAC, WAV, Opus, Vorbis)
 * into a 32-bit float planar AudioClipBuffer.
 */
class AudioDecoder {
public:
    AudioDecoder();
    ...
    /**
     * @brief Quickly inspects audio file metadata without decoding all audio samples.
     */
    static AudioFileInfo probeFile(const std::string &filePath);

    /**
     * @brief Decodes the entire audio stream of a file into a planar 32-bit float AudioClipBuffer.
     ...
     * @return std::shared_ptr<AudioClipBuffer> on success, nullptr on invalid/missing file
     */
    std::shared_ptr<AudioClipBuffer> decodeEntireFile(
