...
inline void convertAndAppend(SwrContext *swrCtx, AudioClipBuffer &clipBuffer,
                             float *const *dstPointers, int maxDstSamples,
                             const uint8_t *const *srcData, int srcSamples) {
  int converted = swr_convert(
      swrCtx, reinterpret_cast<uint8_t **>(const_cast<float **>(dstPointers)),
      maxDstSamples, const_cast<const uint8_t **>(srcData), srcSamples);
...
AudioDecoder::AudioDecoder(AudioDecoder &&other) noexcept
    : m_formatCtx(other.m_formatCtx), m_codecCtx(other.m_codecCtx),
      m_swrCtx(other.m_swrCtx), m_audioStreamIndex(other.m_audioStreamIndex) {
  ...
  if (avformat_find_stream_info(fmtCtx, nullptr) >= 0) {
    for (unsigned int i = 0; i < fmtCtx->nb_streams; ++i) {
      ...
      if (par->codec_type == AVMEDIA_TYPE_AUDIO) {
        ...
        if (fmtCtx->streams[i]->duration > 0 &&
            fmtCtx->streams[i]->time_base.den > 0) {
          info.durationSeconds =
              static_cast<double>(fmtCtx->streams[i]->duration) *
              av_q2d(fmtCtx->streams[i]->time_base);
          info.durationFrames =
              static_cast<int64_t>(info.durationSeconds * info.sampleRate);
...
AudioDecoder::decodeEntireFile(const std::string &filePath,
  ...
  int swrRet = swr_alloc_set_opts2(
      &m_swrCtx, &targetLayout, AV_SAMPLE_FMT_FLTP,
      static_cast<int>(targetSampleRate), &m_codecCtx->ch_layout,
    ...
    if (packet->stream_index == m_audioStreamIndex) {
      if (avcodec_send_packet(m_codecCtx, packet.get()) >= 0) {
        while (avcodec_receive_frame(m_codecCtx, frame.get()) >= 0) {
          convertAndAppend(
              m_swrCtx, *clipBuffer, dstPointers.data(), maxDstSamples,
  ...
  if (avcodec_send_packet(m_codecCtx, nullptr) >= 0) {
    while (avcodec_receive_frame(m_codecCtx, frame.get()) >= 0) {
      convertAndAppend(m_swrCtx, *clipBuffer, dstPointers.data(), maxDstSamples,
  ...
  do {
    ...
    if (flushSamples > 0) {
      clipBuffer->appendFrames(dstPointers.data(),
                               static_cast<size_t>(flushSamples));
    }
