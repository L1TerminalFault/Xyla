...
bool AudioGraph::connect(const std::string &srcNodeId, const std::string &srcPinId,
...
bool AudioGraph::disconnect(const std::string &srcNodeId, const std::string &srcPinId,
                            const std::string &dstNodeId, const std::string &dstPinId) {
...
bool AudioGraph::compile(uint32_t sampleRate, size_t blockSize) {
    ...
    for (const auto &n : m_nodes) {
        if (n) nodeIds.push_back(n->nodeId());
    }

    // Use our decoupled topological sorting engine
    const auto sortedOrder = AudioGraphTopology::computeTopologicalOrder(nodeIds, m_connections);
    ...
    CompiledAudioGraph *oldPtr = m_activeSchedule.exchange(compiledPtr, std::memory_order_acq_rel);
    delete oldPtr;

    return true;
}

void AudioGraph::process(AudioBuffer &hardwareOutput, const AudioClockInfo &clock,
                         AudioBufferPool &pool) noexcept {
    CompiledAudioGraph *schedule = m_activeSchedule.load(std::memory_order_acquire);
    ...
    for (auto &step : schedule->steps) {
        ...
        if (inBuf) {
            ...
            const float mixGain = (incomingConnections > 1)
        ...
        step.node->process(inBuf ? inputList : nullptr, inBuf ? 1 : 0, outputList, 1, ctx);
