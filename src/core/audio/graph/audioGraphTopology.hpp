...
public:
    static bool addConnection(std::vector<GraphConnection>& connections, GraphConnection conn) {
    ...
    static bool removeConnection(std::vector<GraphConnection>& connections, const GraphConnection& target) {
    ...
    static void disconnectAllNodePins(std::vector<GraphConnection>& connections, const std::string& nodeId) {
        connections.erase(
     ...
     * @return Ordered list of node IDs. Returns empty if a cycle/deadlock is detected.
     */
    static std::vector<std::string> computeTopologicalOrder(
