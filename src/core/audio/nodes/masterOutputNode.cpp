...
MasterOutputNode::MasterOutputNode(std::string nodeId)
    ...
    registerPin(AudioPinDescriptor::makeAudioInput("master_in", "In"));
    registerPin(AudioPinDescriptor::makeAudioOutput("hardware_out", "Out"));
    registerPin(AudioPinDescriptor::makeControlInput("master_volume", "Volume", 0.0f, 1.0f, 0.8f));
...
void MasterOutputNode::process(const AudioBuffer *const *inputs,
    ...
    const float targetGain = (m_masterVolIndex >= 0)
        ? getParameterByIndex(static_cast<size_t>(m_masterVolIndex))
        : 0.8f;

    size_t channels = std::min(in->channelCount(), out->channelCount());
    const float* inChannels[2] = { in->channelData(0), channels > 1 ? in->channelData(1) : in->channelData(0) };
    float* outChannels[2] = { out->channelData(0), channels > 1 ? out->channelData(1) : out->channelData(0) };

    auto result = dsp::processMasterBlock(
        inChannels, outChannels, channels, frames,
