...
void WaveformPyramid::generateFromRawPcm(const float *const *channels, size_t channelCount, size_t totalFrames) {
    ...
    for (size_t c = 0; c < m_channelCount; ++c) {
        ...
        // Level 0 — 64 samples per bin
        m_level0[c] = dsp::buildPeakBinsFromPcm(channels[c], totalFrames, kLevel0BinSize);
...
std::vector<AudioPeak> WaveformPyramid::getPeaks(size_t channel,
    ...
    return dsp::slicePeaksForPixels(*sourceLevel, binSize, startSample, sampleCount, targetPixels);
}
