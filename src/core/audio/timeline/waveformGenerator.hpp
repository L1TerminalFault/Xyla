...
public:
    ...
    void generateFromPcm(const AudioClipBuffer &buffer);
    void generateFromRawPcm(const float *const *channels, size_t channelCount, size_t totalFrames);

    /**
     * Slice peaks for [startSample, startSample + sampleCount) at a zoom-driven
     * MIP level. Does not allocate from PCM — only copies from the prebuilt level.
     */
    [[nodiscard]] std::vector<AudioPeak> getPeaks(size_t channel,
                                                  int64_t startSample,
                                                  size_t sampleCount,
    ...
    [[nodiscard]] const std::vector<AudioPeak> &level0(size_t channel) const { return m_level0.at(channel); }
    [[nodiscard]] const std::vector<AudioPeak> &level1(size_t channel) const { return m_level1.at(channel); }
    [[nodiscard]] const std::vector<AudioPeak> &level2(size_t channel) const { return m_level2.at(channel); }
