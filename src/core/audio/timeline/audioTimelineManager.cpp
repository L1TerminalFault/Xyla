...
void AudioTimelineManager::bindTimelineModel(TimelineModel *model,
  ...
  // Listen to TimelineModel modifications to keep audio playback graph synchronized
  if (m_timelineModel) {
    connect(m_timelineModel, &QAbstractItemModel::dataChanged, this,
            &AudioTimelineManager::syncTracksFromModel);
    connect(m_timelineModel, &QAbstractItemModel::rowsInserted, this,
...
AudioTimelineManager::loadAssetAudio(const std::string &assetId,
  ...
  if (buffer && buffer->totalFrames() > 0) {
    ...
    // Asynchronously or synchronously populate waveform cache for the UI timeline
    audio::WaveformGenerator::instance().getOrGenerate(assetId, buffer);
  } else {
    XYLA_LOG_ERROR(
        "AudioTimelineManager",
...
AudioTimelineManager::collectDesiredAudioTracks(
  ...
  std::vector<DesiredTrack> desired;
  if (!m_timelineModel) return desired;
...
void AudioTimelineManager::pruneObsoleteEngineTracks(
  ...
  // Remove nodes from audio graph if their corresponding track was deleted in Qt model
  for (const std::string &nodeId : existingTrackIds) {
    if (liveTrackIds.count(nodeId) == 0) {
      engine.removeTrack(nodeId);
    }
...
void AudioTimelineManager::setupTrackAudioNodes(const DesiredTrack &desiredTrack,
                                                AudioTrackBinding &binding) {
  auto &engine = AudioEngine::instance();
  ...
  // 3. Bind PCM reader lambda using stable trackId (resilient against track reordering)
...
AudioTimelineManager::buildClipRefsForTrack(int trackIndex) const {
  ...
  if (!m_timelineModel) return clipRefs;

  auto *track = m_timelineModel->getTrack(trackIndex);
  if (!track) return clipRefs;
...
AudioTimelineManager::findBindingById(const std::string &trackId) const noexcept {
  for (const auto &tb : m_trackBindings) {
    if (tb.trackId == trackId) {
      return &tb;
    }
...
AudioTimelineManager::tryGetCachedBuffer(const std::string &assetId) noexcept {
  // In the real-time audio thread, try_lock prevents priority inversion / thread blocking
  if (!m_cacheMutex.try_lock()) {
    return nullptr;
  }
...
size_t AudioTimelineManager::renderSingleClip(const AudioTimelineClipRef &clip,
  ...
  const ClipOverlapWindow window = ClipOverlapWindow::compute(
  ...
  float *sliceOutputs[16];
  for (size_t c = 0; c < channelCount && c < 16; ++c) {
    sliceOutputs[c] = outputChannels[c] ? (outputChannels[c] + window.destOffset) : nullptr;
  }
  ...
  if (framesRead > 0) {
    ...
    const ClipDspParams dspParams{clip.volume, clip.pan, clip.channelMode, clip.isMuted};
...
size_t AudioTimelineManager::renderTrackAudioInternal(const AudioTrackBinding &track,
                                                      int64_t timelineSample,
                                                      size_t numFrames,
                                                      float **outputChannels,
                                                      size_t channelCount) noexcept {
  ...
  for (const auto &clip : track.clips) {
    totalRendered += renderSingleClip(clip, blockStart, blockEnd, outputChannels, channelCount);
  }
...
size_t AudioTimelineManager::readTrackAudio(int trackIndex,
                                            ...
                                            float **outputChannels,
                                            size_t channelCount) noexcept {
  // Audio thread must never block on m_tracksMutex: use try_lock to avoid audio dropouts
  if (!m_tracksMutex.try_lock()) {
    return 0;
  }
  ...
  if (targetTrack) {
    // Delegates to unified render function with full DSP (fixing previous missing DSP in this overload)
    framesRead = renderTrackAudioInternal(*targetTrack, timelineSample, numFrames,
