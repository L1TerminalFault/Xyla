 ...
 * Formula: threshold + (1 - threshold) * tanh((s - threshold) / (1 - threshold))
 */
 ...
 */
inline float computeMicroFadeFactor(int remainingFrames, int totalFadeFrames) noexcept {
    if (totalFadeFrames <= 0 || remainingFrames <= 0) return 1.0f;
    float progress = 1.0f - (static_cast<float>(remainingFrames) / static_cast<float>(totalFadeFrames));
...
/**
 * @brief Real-time safe block processor for master gain, micro-fade, and saturation.
 * Pure C++20: Zero allocations, zero external dependencies.
 */
inline MasterDspBlockResult processMasterBlock(
    ...
    float startGain,
    float targetGain,
    int initialFadeFrames,
    int totalFadeWindow = 144,
    float ceiling = 0.95f) noexcept 
{
    ...
    const float stepGain = (frameCount > 0) ? (targetGain - startGain) / static_cast<float>(frameCount) : 0.0f;
    ...
    for (size_t c = 0; c < channelCount; ++c) {
        ...
        for (size_t i = 0; i < frameCount; ++i) {
            ...
            if (c == 0 && absVal > maxPeakL) maxPeakL = absVal;
            if (c == 1 && absVal > maxPeakR) maxPeakR = absVal;
        }
    }

    if (fadeStart > 0) {
        result.remainingFadeFrames = std::max(0, fadeStart - static_cast<int>(frameCount));
    }
    result.finalGain = targetGain;
