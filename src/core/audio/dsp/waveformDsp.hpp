 ...
 */
inline std::vector<AudioPeak> buildPeakBinsFromPcm(const float *pcm, size_t totalSamples, size_t binSize) {
...
/**
 * @brief Downsamples an existing peak pyramid level by factor of 4 (Level 0 -> Level 1 -> Level 2).
 */
inline std::vector<AudioPeak> reducePeakBins(const std::vector<AudioPeak> &sourceBins, size_t reductionFactor = 4) {
...
/**
 * @brief Slices peak bins for [startSample, startSample + sampleCount) onto a target pixel count.
 */
inline std::vector<AudioPeak> slicePeaksForPixels(
    const std::vector<AudioPeak> &levelPeaks,
    size_t binSize,
    int64_t startSample,
    size_t sampleCount,
    size_t targetPixels) 
{
    ...
    const double samplesPerPixel = static_cast<double>(sampleCount) / static_cast<double>(targetPixels);

    std::vector<AudioPeak> result(targetPixels);

    for (size_t px = 0; px < targetPixels; ++px) {
        const int64_t segStart = startSample + static_cast<int64_t>(px * samplesPerPixel);
        const int64_t segEnd = startSample + static_cast<int64_t>((px + 1) * samplesPerPixel);

        size_t bin0 = static_cast<size_t>(std::max<int64_t>(0, segStart / static_cast<int64_t>(binSize)));
        size_t bin1 = static_cast<size_t>(
            (std::max<int64_t>(segEnd - 1, 0) + static_cast<int64_t>(binSize) - 1) / static_cast<int64_t>(binSize));
