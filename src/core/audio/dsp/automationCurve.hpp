...
struct AutomationPoint {
  ...
  bool operator==(const AutomationPoint &other) const noexcept {
    return sampleOffset == other.sampleOffset && value == other.value &&
           type == other.type && tension == other.tension;
  }
};
...
/**
 * @brief Evaluates normalized interpolation between two values given curve type
 * and tension.
 ...
 */
inline float interpolateCurve(float t, float v1, float v2, CurveType type,
...
public:
  ...
  void addPoint(int64_t sampleOffset, float value,
    ...
    m_points.push_back(
        {sampleOffset, value, type, std::clamp(tension, -1.0f, 1.0f)});
  ...
  /**
   * @brief Populates an entire audio block of sample values for sample-accurate
   * modulation.
   */
  void evaluateBlock(float *destination, size_t frameCount,
                     int64_t startSample) const noexcept {
