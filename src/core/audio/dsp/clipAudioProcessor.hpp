 ...
 * @brief Standalone, real-time safe audio DSP calculations and window helpers
 * for timeline clips.
 *
 * Separating this module eliminates massive code duplication between
 * readTrackAudio() and readTrackAudioById(), isolates pure math/DSP from
 * Qt/graph logic, and allows deterministic unit testing of volume, pan law,
 * channel remapping, and overlap math.
 */
...
enum class ChannelMode : int {
  ...
  MonoLeft =
      1, ///< Duplicate Left channel onto Right (Left channel as dual-mono)
  MonoRight =
      2,   ///< Duplicate Right channel onto Left (Right channel as dual-mono)
 ...
 *
 * Translates UI/TimelineModel frame coordinates (FPS-based) into absolute PCM
 * sample positions (sampleRate-based) for sample-accurate scheduling on the
 * audio render thread.
 */
struct AudioTimelineClipRef {
  std::string clipId; ///< Unique identifier of the timeline clip item
  std::string
      assetId; ///< Key referencing the decoded PCM buffer in m_assetCache
  int64_t startSample{
      0}; ///< Timeline position where clip playback starts (in samples)
  int64_t durationSamples{
      0}; ///< Active playback duration on timeline (in samples)
  int64_t sourceInSample{
      0}; ///< Start trim offset within source media asset (in samples)

  float volume{1.0f}; ///< Static gain multiplier [0.0, ...]
  float pan{0.0f};    ///< Stereo pan position [-1.0 = hard Left, 0.0 = Center,
                      ///< +1.0 = hard Right]
  int channelMode{
      0}; ///< Routing: 0 = Stereo, 1 = Mono Left, 2 = Mono Right, 3 = Mute
  bool isMuted{false}; ///< Explicit clip mute flag
};

/**
 * @brief Represents an active audio track binding connecting the UI model to
 * the AudioEngine graph.
 *
 * Each binding pairs a TimelineModel track with a MixerTrackNode
 * (gain/pan/summing bus) and a ClipSourceNode (PCM reader provider callback) in
...
struct AudioTrackBinding {
  ...
  ClipSourceNode *sourceNode{
      nullptr}; ///< Source generator node invoking PCM read callbacks
  std::vector<AudioTimelineClipRef>
      clips; ///< Snapshot of clips placed on this track
};

/**
 * @brief Calculates sample-accurate intersection windows between an audio
 * engine render block and an individual timeline clip.
 */
struct ClipOverlapWindow {
  ...
  int64_t bufferOffset =
      0; ///< Start sample offset inside the source AudioClipBuffer
  size_t destOffset =
      0; ///< Frame offset inside the engine render block's output array
   ...
   * @param clipSourceIn Sample offset in the source asset where playback starts
   * (trim in-point)
  ...
  static inline ClipOverlapWindow compute(int64_t blockStart, int64_t blockEnd,
    ...
    window.overlapFrames =
        static_cast<size_t>(window.overlapEnd - window.overlapStart);

    // Account for timeline position relative to clip start, plus the user's
    // source trim in-point
...
/**
 * @brief Applies channel mode remapping, volume gain, and linear stereo pan law
 * in-place.
 *
 ...
 *
 * @param sliceOutputs Array of float pointers pointing to destination channel
 * buffers
 ...
 */
inline void applyClipDsp(float **sliceOutputs, size_t channelCount,
                         size_t framesRead,
  ...
  if (channelCount >= 2 && sliceOutputs[0] && sliceOutputs[1]) {
    ...
    // Scale any additional surround or auxiliary channels (ch 2..15) by master
    // volume
...
/**
 * @brief Computes stereo left and right gains based on master volume and linear
 * pan [-1.0, +1.0].
 */
inline void computePanGains(float volume, float pan, float &outGainL,
                            float &outGainR) noexcept {
 ...
 */
inline int64_t framesToSamples(int64_t frames, double fps,
                               uint32_t sampleRate) noexcept {
  if (fps <= 0.0)
    return 0;
  return static_cast<int64_t>(static_cast<double>(frames) *
                              (static_cast<double>(sampleRate) / fps));
}

/**
 * @brief Converts audio sample count to video timeline frame count.
 */
inline int64_t samplesToFrames(int64_t samples, double fps,
                               uint32_t sampleRate) noexcept {
  if (sampleRate == 0)
    return 0;
  return static_cast<int64_t>(static_cast<double>(samples) *
                              (fps / static_cast<double>(sampleRate)));
}
