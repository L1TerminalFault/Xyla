 ...
 */
inline std::pair<float, float> computeTargetGains(const MixerTrackParams& params) noexcept {
    if (params.isMuted) {
        return {0.0f, 0.0f};
    }
    ...
    return {
        targetPanL * params.volume * phaseMod,
        targetPanR * params.volume * phaseMod
    };
}

/**
 * @brief Real-time planar DSP loop for channel swap, M/S width, ramped gain, and metering.
 * Pure C++20: Zero heap allocations, SIMD-friendly loop.
 */
inline MixerMeterResult processMixerTrackBlock(
    const float* const* inputs,
    float* const* outputs,
    size_t inChannels,
    size_t outChannels,
    size_t frameCount,
    const MixerTrackParams& params,
    float lastGainL,
    float lastGainR) noexcept
{
    ...
    const float stepGainL = (targetGainL - lastGainL) / static_cast<float>(frameCount);
    const float stepGainR = (targetGainR - lastGainR) / static_cast<float>(frameCount);
