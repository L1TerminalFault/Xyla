...
public:
    ...
    [[nodiscard]] const char *backendName() const noexcept override { return "DummyMock"; }

    bool initialize(const AudioDeviceConfig &config, IAudioRenderCallback *callback) override {
        m_config = config;
        m_callback = callback;
     ...
     */
    void stepTick() {
        if (!m_callback || !isRunning()) return;
