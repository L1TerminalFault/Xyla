...
bool PipeWireAudioBackend::start() {
    ...
    struct pw_properties *props = pw_properties_new(
        ...
        PW_KEY_APP_NAME, "Xyla NLE",
        PW_KEY_NODE_NAME, "xyla_audio_master",
        PW_KEY_NODE_LATENCY, latencyStr.c_str(),
        nullptr
    );
    ...
    if (res < 0) {
        XYLA_LOG_ERROR("PipeWire", "pw_stream_connect failed: " + std::string(strerror(-res)));
        return false;
    }
...
void PipeWireAudioBackend::onProcess() {
    ...
    clockInfo.timelineSamplePosition = AudioMasterClock::instance().timelineSamples();
    ...
    clockInfo.timelineSeconds = static_cast<double>(clockInfo.timelineSamplePosition) /
    ...
    const float *channelPointers[2] = {
        ...
        (channels > 1) ? m_pipewireBuffer.channelData(1) : m_pipewireBuffer.channelData(0)
    };

    // Use our decoupled real-time interleaver
    hal::PipeWireUtils::interleavePlanarToPacked(
        channelPointers, channels, dstInterleaved, n_frames);

    if (clockInfo.isPlaying) {
        AudioMasterClock::instance().updateFromRenderCallback(
            clockInfo.timelineSamplePosition, n_frames, m_config.format.sampleRate, true);
    }
