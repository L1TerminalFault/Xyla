...
bool AudioEngine::initialize(std::unique_ptr<IAudioDeviceBackend> backend,
    ...
    m_format = config.format;
    m_bufferSize = config.bufferSizeFrames;
    m_isOffline.store(false, std::memory_order_relaxed);

    // Scratchpad buffer pool: 32 buffers, channelCount, maxFrames
    m_bufferPool.initialize(32, m_format.channelCount, m_bufferSize);
...
bool AudioEngine::initializeOffline(const AudioFormat &format, uint32_t bufferSize) {
    shutdown();

    m_backend.reset();
    m_format = format;
    m_bufferSize = bufferSize;
    m_isOffline.store(true, std::memory_order_relaxed);
...
bool AudioEngine::start() {
    ...
    if (m_isOffline.load(std::memory_order_acquire)) {
        return true;
    }
    return m_backend && m_backend->start();
}
...
bool AudioEngine::stop() {
    ...
    return m_backend ? m_backend->stop() : false;
}
...
void AudioEngine::shutdown() {
    ...
    m_isInitialized.store(false, std::memory_order_release);
    m_isOffline.store(false, std::memory_order_release);
}

bool AudioEngine::isRunning() const noexcept {
    if (m_isOffline.load(std::memory_order_relaxed)) {
...
bool AudioEngine::removeTrack(const std::string &trackId) {
    // 1. Remove from local tracking vector
    m_tracks.erase(
        std::remove_if(m_tracks.begin(), m_tracks.end(),
                       [&](MixerTrackNode *n) { return n->nodeId() == trackId; }),
        m_tracks.end());

    // 2. Disconnect and remove the MixerTrackNode
    m_graph.disconnectAll(trackId);
    m_graph.removeNode(trackId);

    // 3. Clean up associated source node if present (e.g., source_0 for track_0)
    const std::string sourceNodeId = AudioEngineNaming::deriveSourceNodeId(trackId);
    m_graph.disconnectAll(sourceNodeId);
    m_graph.removeNode(sourceNodeId);
...
void AudioEngine::renderAudio(AudioBuffer &outputBuffer,
    ...
    const bool isScrubbingActive = AudioMasterClock::instance().isScrubbing();
    if (!clock.isPlaying && !isScrubbingActive) {
        return;
    }

    // Real-time non-blocking automation parameter dispatch
    if (m_automationMutex.try_lock()) {
        for (const auto &clip : m_automationClips) {
            AudioNode *target = m_graph.findNode(clip->targetNodeId());
            if (target) {
                clip->applyBlock(target, clock.timelineSamplePosition, clock.bufferSizeFrames);
            }
        }
        m_automationMutex.unlock();
    }
