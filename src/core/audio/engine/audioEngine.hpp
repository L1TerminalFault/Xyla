...
public:
     ...
     */
    bool initializeOffline(const AudioFormat &format = AudioFormat::standardStereo(48000),
    ...
    [[nodiscard]] MasterOutputNode *masterNode() const noexcept { return m_masterNode; }

    MixerTrackNode *addTrack(const std::string &trackId, const std::string &name);
    bool removeTrack(const std::string &trackId);
    const std::vector<MixerTrackNode *> &tracks() const noexcept { return m_tracks; }
